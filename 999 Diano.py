...
def load_image(
 ...
 fullname=os.path.join('resources',s name)
...
def gameover_display_message(rbtn_image,gmo_image):
    rbtn_rect=gmo_image.get_rect()
    rbtn_rect.centerx=width_screen/2
    rbtn _rect top=height_screen*0.52
      
      gmo_rect=gmo_image.get_rect()
      gmo_rect.centerx=width_screen/2
      gmo rect.centery=height screen*0.35
 ...
 def extractDigits(num):
     if num > -1:
         ...
         while(num/10!=0):
             ...
             d.appenf(num%10)
             for i in range(|en(d),5):
                 d.append(0)
                d.reverse()
                return d
                
 class Dino():
     def __init__( self ,sx=-1,sy=-1):!bb
             self.imgs,self.rect=load_sprite_sheet(dino png,5,1,sx,sy,-1)
             self.imgs1,self.rect]-load_sprite_sheet_sheet('dino_ducking.png,'2,1,59,sy,-1)
             self.rect.bollom=int(0.98*height_screen)
             ...
             self.jumpspeed=11.5
                ...
                def checkbounds(self):
                    if self.rect.bollom>int(0.98*height_screen):
                        self.rect.bollom=int(0.98*height_screen)
                        self.jumping=False
   ...
   if not self dead and self.counter%7==6 and self.blinking==False:
                 ...
                 if self.score%100==0 and self.score !=0:
                     if pygame.mixer get_int() != None:
                         soundOnCheckpoint.play()
     self.counter=(self.counter+1)
     
     class Cactus(pygame.sprite.sprite):
                 def__init__(self,speed=5,sx=-1,sy=-1):
                     pygame.sprite.Sprite__init__(self,self.containers)
                     self.imgs,self.rect=load sprite_sheet('cactus-small.png,3,1,sx,sy,-1')
                     ...
                     def draw(self):
                         screenDisplay.blit(self.image,self.movement)
...
class birds(pygame.sprite.Sprite):
     def__init__(self,speed=5,sx=-1,sy=-1):
                 pygame.sprite Sprite__init__(self,self.cointainers)
                 self.imgs,self.rect=load_sprite_sheet('birds.png',2,1,sx,sy,-1)
                 self.birds.height=[height screen*0.82,height_screen*0.75,height_screen*0.60]
                 self.rect.contary=self.birds_height[random.randrange(0,3)]
                 self.rect.left=width_screen+self.rect.width
                 self.image=self.imgs[0]
                 self.movement=[-1*speed,0]
                 self.index=0
...
class Ground():
     def __init__(self,speed=-5):
                    ...
                    self.rect.bottom=height screen
...
class cloud(pygame.sprite.Sprite):
     def __init__(self x,y):
        pygame.sprite.Sprite__init__(self,self.containers)
        self.image,self.rect=load image('cloud.png',int(90*30/42),30,-1)
        self.speed=1
        self.rect.left=x
        self.rect.top=y
...
t_ground,t_ground_rect=load_sprite('ground.png',15,1,-1,-1,-1)
...
while not starting_game:
 ...
 if pygame.display get_surface0!=None:
     ...
     def gameplay():
        ...
        gmo_image,gmo rect=load_image('game_over.png',190,11,-1)
        
        t_images,t_rect=load_sprite_sheet('numbers.png',numbers.png,12,1,11,int(11*6/5),-1)
        ado_image=pygame.Surface((22,int(11*6/5)))
        ado_rect=ado_image.get_rect()
        ado_image.fill(backgroundColor)
        ado_image.blit(t_image[10],t_rect)
                                ...
                                if event.key==pygame.K_SPACE:
                                    if gamer_Dino.rect.bottom==int(0.98*height_screen):
                                        ...
                                        if pygame.mixer.get_init()!=None:
                                            soundOnJump.play()
                                            gamer_Dino.jumping and gamer_Dino.dead):
...
if len(cactusan)<2:
        if len(cactusan)==0:
            ...
            for I in last_end_obs:
                if I.rect.right<width_screen*0.7 and random.randrange(0.50)==10:
                    last_end_obs.empty()
                    last_end_obs.add(Cactus(gp,40,40))
                    
if len(smallBird)==0 and random.randrange(0,200)==10 and counter>500:
        for I in last_end_obs:
            if I.rect.right<width_screen*0.8:
                last_end_obs.empty()
